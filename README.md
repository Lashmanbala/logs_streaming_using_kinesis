...
The requirement is to capture the logs that are generated from a simulated docker application and store it in the s3 bucket in a streming fashion.

Using Amazon Kinesis Data Firehose logs are captured and streamed in real-time and stored in s3.

The logs are generated by a python script using Faker module.This solution is containerized using Docker for easy deployment, and an AWS Kinesis Agent is installed in the container to collect and push logs to the Kinesis Firehose.
